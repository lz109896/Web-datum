## 1：样式的继承与层叠
```
1.浏览器的默认样式：就算一行样式度没有进行设置，也会有样式，是浏览器自带的样式

2.样式的继承：元素内本身没有定义颜色，是父元素定义了颜色为红色，所以会显示颜色，后代元素会继承祖先元素
...
样式的层叠
1.元素声明的样式的权重高于浏览器默认样式
2.浏览器默认样式的权重高于继承的样式


权重越来越低：元素声明的样式 > 元素的浏览器默认样式 > 继承父级元素的样式

```
## 2：样式优先级
```
选择器优先级
!important(最高，不建议使用)-->style-->id选择器-->类选择器-->元素选择器--> *（通用选择器）-->浏览器默认-->继承（最低）
...
1.如果属性是默认继承的，则取父元素的同属性的应用值（computed value）。
通过该方式得到的值我们称之为继承值（inherit value）

2.如果属性是默认不继承的，则取该属性的初始值（initial value）
...
在计算 <span> 元素的最终颜色时，因为该元素本身没有声明 color 属性，且由于 color 属性属于默认继承类型的，
所以会取父元素 color 属性的应用值（红色）。这样 <span> 通过父元素继承过来的值就是红色，
而最后的应用值用得就是该继承值。

当然我们还可以通过显式设置 color 属性的值为继承值，如color: inherit;。
...
初始值针对不同类别的属性具有不同的含义：
1.对于继承属性, 初始值只能被用于没有指定值的根元素（HTML）上
...
具体可参看：Assigning property values, Cascading, and Inheritance。:https://www.w3.org/TR/CSS22/cascade.html#value-stages
（如果你能理解过来，那当然最好了，如果实在理解不了，可以先忽略，以后慢慢的自然就懂了）
```
#### 附
```
默认没有一个官方地址查看所有的继承属性或非继承属性，不过可以单独查看某个属性是否属于继承或非继承属性，
如在 MDN 的参考手册中查看 color 属性，在“是否是继承属性”那一栏就写了“yes”，截图如下：
```
![]()
